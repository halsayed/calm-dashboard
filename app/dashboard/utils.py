...
def project_meter (user: User):
    ...
    if r.status_code == 200:
        ...
        if r.json()['metadata']['length']==0:
            ...
            meter['reserved_memory']=round(int(r.json()['entities'][0]['status']['resources']['reserved']['memory'])/1024/1024/1024)
            current_app.logger.debug('project_meter: reserved_memory={}'.format(meter['reserved_memory']))
            meter['utilized_memory']=round(int(r.json()['entities'][0]['status']['resources']['utilized']['memory'])/1024/1024/1024)
            ...
            meter['reserved_vcpu']=int(r.json()['entities'][0]['status']['resources']['reserved']['vcpu'])
            current_app.logger.debug('project_meter: reserved_vcpu={}'.format(meter['reserved_vcpu']))
            meter['utilized_vcpu']=int(r.json()['entities'][0]['status']['resources']['utilized']['vcpu'])
